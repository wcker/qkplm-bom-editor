...
export function sha256Hex(input: string | Uint8Array): string {
  const source = typeof input === 'string' ? encodeUtf8(input) : input;
  const paddedLength = Math.ceil((source.length + 9) / 64) * 64;
  const padded = new Uint8Array(paddedLength);
  padded.set(source);
  padded[source.length] = 0x80;

  const bitLengthHigh = Math.floor(source.length / 0x20000000);
  const bitLengthLow = (source.length << 3) >>> 0;
  writeUint32BigEndian(padded, paddedLength - 8, bitLengthHigh);
  writeUint32BigEndian(padded, paddedLength - 4, bitLengthLow);
